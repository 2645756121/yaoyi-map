...
const EDGE_PROFILE = mkdtempSync(join(tmpdir(), 'edge-resp-'));
...
async function main() {
  ...
  for (const device of devices) {
    ...
    await sleep(2000); // 等待响应式重排

    // 截图
    const r = await sendCmd(ws, 'Page.captureScreenshot', { format: 'png' });
    writeFileSync(resolve(REPORT_DIR, `responsive-${device.name}.png`), Buffer.from(r.data, 'base64'));
    console.log(`Screenshot saved: responsive-${device.name}.png (${device.width}x${device.height})`);
  }
